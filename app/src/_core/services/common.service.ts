...
export class CommonService {
  ...
  mark_yummysaver_refund(_fund: any): Observable<any> {
    return this.http.post<any>(Yummysaver+'/refunded', _fund, { headers: this.httpUtils.getHTTPHeaders() }).pipe(catchError(this.httpUtils.errorHandler));
  }
...
fetch_product_rating(_user: any): Observable<Result> {
  ...
  check_referel_code(_user: any): Observable<Result> {
    return this.http.get<Result>(USER_URL + `/referel/${_user}`,{ headers: this.httpUtils.getHTTPHeaders() }).pipe(catchError(this.httpUtils.errorHandler));
  }
