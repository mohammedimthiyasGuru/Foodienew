...
const CHATS_URL_CREATE = RemoteConfig.BASE_URL + '/chats/msgs2';


const CHATS_MARK_URL = RemoteConfig.BASE_URL + '/chats/markasread';
...
export class ChatService {
    ...
    markAsRead(sender, receiver): Observable<any> {
        return this.http.get<any>(CHATS_MARK_URL + `/${sender}/${receiver}`, { headers: this.httpUtils.getHTTPHeaders() }).pipe(catchError(this.httpUtils.errorHandler));
    }

    sendMsg(_msg: any): Observable<any> {
        return this.http.post<any>(CHATS_URL_CREATE, _msg, { headers: this.httpUtils.getHTTPHeaders() }).pipe(catchError(this.httpUtils.errorHandler));
    }
}
