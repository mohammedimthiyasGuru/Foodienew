...
export class VideoService {
    ...
    create(_docVideo: File, _data: any): Observable<any> {
      console.log(_docVideo);
        ...
        return this.http.post<any>(VIDEO_URL, formData, { headers: this.httpUtils.httpHeaders() }).pipe(catchError(this.httpUtils.errorHandler));
    }

    view(_videoId: number,): Observable<any> {
        return this.http.get<any>(VIDEO_URL + `/view/${_videoId}`, { headers: this.httpUtils.getHTTPHeaders() }).pipe(catchError(this.httpUtils.errorHandler));
    }
